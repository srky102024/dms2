...
const dbName = "lab3";
const collectionName = "5770_Vinay"; // Update this to your specific collection name
...
function initializeIndexedDBData() {
    for (let i = 1000; i < 2000; i++) { // Start from 1000 to ensure unique UUIDs
        const uuid = `uuid-${i}`;
        indexedDBData[uuid] = {
            uuid: uuid,
...
async function syncMongoDBToIndexedDB() {
    ...
    let recordsUpdatedInIndexedDB = 0;
    ...
    try {
        ...
        mongoData.forEach((doc) => {
            if (!indexedDBData[doc.uuid]) {
                ...
                recordsUpdatedInIndexedDB++;
            }
        });

        console.log(`MongoDB to IndexedDB Sync Summary: ${recordsAddedToIndexedDB} records added, ${recordsUpdatedInIndexedDB} records updated in IndexedDB.`);
    } catch (error) {
...
async function syncIndexedDBToMongoDB() {
    ...
    try {
        ...
        // Log existing MongoDB UUIDs
        const mongoUUIDs = mongoData.map(doc => doc.uuid);
        console.log("Existing MongoDB UUIDs:", mongoUUIDs);

        const newDataToMongoDB = indexedDBEntries.filter(
            (indexedDoc) => !mongoUUIDs.includes(indexedDoc.uuid)
        );
